    ...
    describe("authenticate user", function () {
        it("signup user", function (done) {
            ...
            authenticateSignup(req, "new_user", "new_password", (err, user) => {
                ...
                chai_1.assert.equal("new_user", user.username);
                chai_1.assert.equal("first", user.firstname);
                chai_1.assert.equal("last", user.lastname);
                return models.User.findOne({ username: "new_user" })
                    .then(foundUser => {
                    chai_1.assert.equal("new_user", foundUser.username);
                    chai_1.assert.equal("first", foundUser.firstname);
                    chai_1.assert.equal("last", foundUser.lastname);
                    done();
                })
                    .catch(done);
            });
        ...
        it("signup with existing username", function (done) {
            ...
            authenticateSignup(req, "test", "new_password", (err, user) => {
                chai_1.assert.equal(null, err);
                chai_1.assert.equal(false, user);
                done();
            });
        });
        it("authenticate user success", function (done) {
            authenticate("TEST", "test1234", (err, user) => {
                chai_1.assert.equal(null, err);
                chai_1.assert.notEqual(null, user);
                chai_1.assert.notEqual(undefined, user);
                chai_1.assert.notEqual(false, user);
                done(err);
            });
        });
                ...
                authenticate("test", "abc1234", (err, user) => {
                    ...
                    authenticate("test", "abc1234", (err, user) => {
                        ...
                        models.User.findOne({ username: "test" })
                            .then(foundUser => {
                            chai_1.assert.equal(3, foundUser.badPasswordAttempts);
                            chai_1.assert.equal(true, foundUser.isLockedOut);
                            return done(err);
                        })
                            .catch(done);
                    });
        ...
        it("successful login resets bad password attempts", function (done) {
            models.User.findOneAndUpdate({ username: "test" }, { $set: { badPasswordAttempts: 2 } }, { new: true })
                .then(updatedUser => {
                chai_1.assert.equal(false, updatedUser.isLockedOut);
                chai_1.assert.equal(2, updatedUser.badPasswordAttempts);
                authenticate("test", "test1234", (err, user) => {
                    chai_1.assert.equal(null, err);
                    chai_1.assert.notEqual(false, user);
                    chai_1.assert.equal(0, user.badPasswordAttempts);
                    return done();
                });
            })
                .catch(done);
    ...
    describe("authenticate client", function () {
        it("success", function (done) {
            authenticateClient("test_id", "test_secret", (err, client) => {
                chai_1.assert.equal(null, err);
                chai_1.assert.notEqual(null, client);
                chai_1.assert.notEqual(undefined, client);
                chai_1.assert.notEqual(false, client);
                done(err);
            });
        });
        it("failure", function (done) {
            authenticateClient("test_id", "bad_secret", (err, client) => {
                chai_1.assert.equal(null, err);
                chai_1.assert.equal(false, client);
                done(err);
            });
        });
    });
    ...
    describe("oauth2", function () {
        it("grant code success", function (done) {
            Promise.all([models.Client.findOne({}),
                ...
                .then(code => {
                chai_1.assert.notEqual(null, code);
                done();
            })
                .catch(err => {
                done(err);
        ...
        it("exchange code success", function (done) {
            Promise.all([models.Client.findOne({}),
                ...
                .then(tokens => {
                chai_1.assert.notEqual(null, tokens.accessToken);
                chai_1.assert.equal(null, tokens.refreshToken);
                chai_1.assert.notEqual(null, tokens.idToken);
                const date = new Date();
                const decodedIdToken = jwt.decode(tokens.idToken, tokens.client.secret);
                chai_1.assert.equal(tokens.client.clientIdentifier, decodedIdToken.aud);
                chai_1.assert.isBelow(date.getTime(), new Date(decodedIdToken.exp).getTime());
                done();
            })
                .catch(err => {
                done(err);
        ...
        it("exchange code bad client code", function (done) {
            Promise.all([models.Client.findOne({}),
                ...
                .then(tokens => {
                chai_1.assert.equal(false, tokens.accessToken);
                chai_1.assert.equal(undefined, tokens.refreshToken);
                chai_1.assert.equal(undefined, tokens.idToken);
                done();
            })
                .catch(err => {
                done(err);
