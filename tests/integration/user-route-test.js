...
describe("integration: user routes", function () {
    ...
    it("get many users", function (done) {
        request.get('/api/v1/users')
            ...
            chai_1.assert.equal(1, res.body.users.length);
            done(err);
        });
    });
    it("get one user", function (done) {
        models.User.find()
            ...
            request.get('/api/v1/users/' + userId)
                ...
                .end(function (err, res) {
                chai_1.assert.equal(null, err);
                chai_1.assert(Array.isArray(res.body.clients));
                chai_1.assert.equal(userId, res.body.user._id);
                done(err);
            });
        })
            .catch(done);
    ...
    it("put user", function (done) {
        ...
        models.User.find()
            ...
            request.put('/api/v1/users/' + userId)
                ...
                chai_1.assert.equal(null, err);
                chai_1.assert.equal(userId, res.body.user._id);
                chai_1.assert.equal(newFirstName, res.body.user.firstname);
                done(err);
            });
        })
            .catch(done);
